...
(function (root, factory) {
  ...
  const VERSION = 2, WIDTH = 13, HEIGHT = 11;
  ...
  function dawn(s) {
    ...
    if (s.config.threat > 0 && random(s) < s.config.threat / 700) {
      const hit = damage(s, Math.max(0, 11 - s.camp.barricade * 4 - countRole(s,'guard') * 2 - S.benefits(s).defense));
  ...
  function act(s, action, args = {}) {
    ...
    const t = s.map[s.position]; let hours = 0, completingStage=0, completingSearch=false, injured=false;
    switch (action) {
      case 'move':
  ...
  function validate(s, legacy=false) {
    ...
    if (!object(s) || s.version !== (legacy?1:VERSION) || !object(s.config)) fail();
    if (!legacy) {
    ...
    if (!object(s.camp) || !Object.keys(structures).every(k=>int(s.camp[k],0,legacy?structures[k].max:S.maxLevel(s,k))) || Object.keys(s.camp).some(k=>!Object.hasOwn(structures,k))) fail();
    if (!Array.isArray(s.crew) || s.crew.length<1 || s.crew.length>(legacy?2+2*s.camp.beds:capacity(s))) fail();
    if(!legacy && s.recruitment && (s.crew.length>=capacity(s) || !s.camp.radio || (!s.over && (s.recruitment.dueAt<=S.absoluteHour(s) || s.recruitment.dueAt>S.absoluteHour(s)+12)))) fail();
    const ids = new Set();
  ...
  function deserialize(text) {
    ...
    if(s && s.version===1) {
      validate(s,true);
      s.version=VERSION; s.baseStage=1; s.recruitment=null; s.nextRecruitAt=0;
  ...
  return {VERSION,WIDTH,HEIGHT,systems:S,defs:{stages:S.stages,scenarios,eras,origins,professions,regions,tiles,structures,eventDefs,roles,resources,defaults},newGame,act,status,resolveEvent,choiceStatus,costToBuild,tradeCost,production,consumption,danger,capacity,atHome,neighbors,distance,tune,serialize,deserialize,validate};
});
