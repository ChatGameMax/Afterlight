...
(function (root, factory) {
  ...
  'use strict';
  const S = typeof module === 'object' && module.exports ? require('./systems.js') : globalThis.AfterlightSystems;
  const A = typeof module === 'object' && module.exports ? require('./armory.js') : globalThis.AfterlightArmory;
  const P = typeof module === 'object' && module.exports ? require('./expeditions.js') : globalThis.AfterlightExpeditions;
  const VERSION = 4, WIDTH = 13, HEIGHT = 11;
  ...
  const countRole = (s, r) => P.homeCrew(s).filter(c => c.role === r).length;
  ...
  function addResources(s, amounts, factor = 1, recovery = false) {
    for (const key of resources) if (amounts[key]) {
      const before=s.resources[key];s.resources[key]=clamp(before+Math.round(amounts[key]*factor),0,9999);
      const delta=s.resources[key]-before;
      if(delta<0)P.stockDebit(s,key,-delta);else if(recovery)P.fieldCredit(s,key,delta);
    }
  }
  ...
  function recruit(s) {
    ...
    const c = {id: s.nextCrew++, name: names.length ? pick(s,names) : `${pick(s,firstNames)} ${s.nextCrew}`, profession: pick(s, Object.keys(professions)), role: 'forager', trait: pick(s, traits)};
    s.crew.push(c); return c;
  }
  function newGame(input) {
    const config = normalizeConfig(input);
    const s = {version: VERSION, armory:A.initial(), baseStage: 1, recruitment: null, nextRecruitAt: 0, config, rng: hash(config.seed), day: 1, hour: 8, health: 100, morale: 72, fatigue: 0, noise: 0, resources: {food: 12, water: 15, scrap: 14, medicine: 3, fuel: 3}, crew: [], nextCrew: 1, camp: {beds: 1, barricade: 0, water: 0, garden: 0, workshop: 0, radio: 0}, map: [], base: 0, position: 0, weather: 'clear', fronts: [], factions: [{name: 'The Commons', rep: 15, power: 45}, {name: 'The Exchange', rep: 25, power: 40}, {name: 'The Iron Choir', rep: -30, power: 35}], signal: 0, won: false, over: false, event: null, lastEvent: -5, actionCount: 0, log: [], stats: {searched: 0, built: 0, recruited: 0, broadcasts: 0, survived: 0}};
    ...
    s.armor=P.inventory();s.expedition=P.initial(s);
  ...
  function danger(s, id = s.position) {
    ...
    if (id === s.base) n -= 10 + s.camp.barricade * 13 + countRole(s, 'guard') * 4 + S.benefits(s).defense;
  ...
  function encounter(s, factor = 1) {
    if (random(s) * 100 < danger(s) * .5 * factor) {
      const response=P.respond(s,integer(s, 4, 13) + s.fatigue / 18);
      const hit = damage(s,response.damage);
      log(s,`${A.weapons[response.key].name}: ${response.used?response.used+' rounds spent':'no ammo spent'}; +${response.noise} noise${response.reason?'; '+response.reason:''}.`, 'info');
  ...
  function production(s) {
    const bonus=S.benefits(s).output;
    return {food: s.camp.garden * 3 + (s.camp.garden ? countRole(s, 'grower') * 2 : 0) + countRole(s, 'forager') + bonus.food, water: s.camp.water * (s.weather === 'rain' ? 6 : 4) + bonus.water, scrap: s.camp.workshop * 2 + bonus.scrap, medicine: bonus.medicine, fuel: bonus.fuel};
  }
  ...
  function dawn(s) {
    ...
    const supply=A.daily(s);for(const [key,n] of Object.entries(supply))if(n)A.addAmmo(s,key,n);
    if(s.armory.level)log(s,`Supply station sorted ${supply.light} pistol rounds, ${supply.shells} shells and ${supply.rifle} rifle rounds.`, 'good');
    ...
    if (foodMissing || waterMissing) {
      ...
      log(s, `Day ${s.day}. Camp produced ${output.food} food, ${output.water} water, ${output.scrap} scrap, ${output.medicine} medicine, ${output.fuel} fuel; consumed ${need.food} food and ${need.water} water.`, 'good');
    }
    if (s.over) return;
    if (s.config.scenario === 'pandemic' && s.day % 3 === 0) {
      if (s.resources.medicine > 0) {addResources(s,{medicine:1},-1); log(s, 'Community care used 1 medicine to protect the group.', 'info');}
    ...
    if (s.config.threat > 0 && random(s) < s.config.threat / 700) {
      const impact=Math.max(0,11 - s.camp.barricade * 4 - countRole(s,'guard') * 2 - S.benefits(s).defense);
      const response=P.respond(s,impact,true),hit=damage(s,response.damage);
      if(response.used)log(s,`Perimeter response used ${response.used} rounds across the home defenders; +${response.noise} noise.`);
      log(s, `Your perimeter was tested overnight. ${hit ? `${hit} health lost.` : 'The defenders held the line.'}`, hit ? 'bad' : 'good');
    }
  }
  function arrive(s) {
    if (!s.over && s.recruitment && S.absoluteHour(s)>=s.recruitment.dueAt) {
      s.recruitment=null;
      const person=recruit(s); s.stats.recruited++; s.nextRecruitAt=S.absoluteHour(s)+12;
      log(s, `${person.name} arrived at home in response to your recruitment call. Assign a role in Survivors; daily food and water needs have increased.`, 'good');
    }
  }
  ...
  function maybeEvent(s) {
    ...
    if (random(s) < .26) {
      ...
      if (S.freeBeds(s)>0) list.push('stranger');
  ...
  function status(s, action, args = {}) {
    ...
    const expeditionStatus=P.status(s,action,args);if(expeditionStatus!==null)return expeditionStatus;
    const equipmentStatus=A.status(s,action,args);if(equipmentStatus!==null)return equipmentStatus;
    const t = s.map[s.position];
    if (action === 'move') {
      if (!Number.isInteger(args.id) || args.id < 0 || args.id >= s.map.length) return 'Choose a valid district.';
      if (!S.canStep(s,s.position,args.id)) return 'Choose adjacent dry land, including diagonals. Diagonal travel cannot cut flooded corners.';
    } else if (action === 'travel') {
      const route=S.planRoute(s,args.id); if(!route.ok) return route.message;
      if(s.health<=35 || s.fatigue>=80) return 'Rest or treat wounds before a long journey. Single-step travel remains available.';
      ...
      if (s.camp[args.key] >= S.maxLevel(s,args.key)) return 'Already at maximum level.';
      ...
      if (!atHome(s)) return 'Return to home base to expand.';
      if (s.baseStage>=5) return 'Haven is the final base stage.';
      if (S.stageRequirements(s).some(r=>!r.met)) return 'Complete the next-stage requirements shown below.';
      if (!afford(s,S.stages[s.baseStage+1].cost)) return 'Not enough supplies for the next base stage.';
    } else if (action === 'recruit' || action === 'seekSurvivors') {
      if (!atHome(s)) return 'Coordinate recruitment at home base.';
      if (s.recruitment) return 'A survivor is already on the way. Their bed is reserved.';
      if (S.freeBeds(s)<1) return 'No free beds. Upgrade living quarters or expand the base first.';
      if (S.absoluteHour(s)<s.nextRecruitAt) return `Recruitment available in ${s.nextRecruitAt-S.absoluteHour(s)} game hours.`;
      if (action==='recruit' && !s.camp.radio) return 'Build a radio relay, or organize a survivor search instead.';
      if (!afford(s,action==='recruit'?{food:4,water:3,fuel:1}:{food:4,water:3})) return 'Recruitment needs 4 food, 3 water, and (for radio calls) 1 fuel.';
  ...
  function tradeCost(s,key) {const price = {food:5, water:4, medicine:6, fuel:5}[key] || 5; return {scrap: Math.max(2,price - (s.config.profession === 'negotiator' ? 1 : 0) - (s.factions[1].rep >= 50 ? 1 : 0)) * (s.baseStage || 1)};}
  function checkWin(s) {
  ...
  function act(s, action, args = {}) {
    ...
    if (action==='travel') {
      const route=S.planRoute(s,args.id), moved=[], transitions=[];
      let stop='destination reached';
      for (const id of route.path) {
        const health=s.health, previousHour=s.hour;
        const result=act(s,'move',{id});
        if(!result.ok) {stop=result.message; break;}
        moved.push(id);transitions.push(...(result.transitions||[]));
        if(s.over) {stop='expedition ended';break;}
        if(s.event) {stop='resolve the encounter';break;}
        if(result.injured || s.health<health) {stop='the group took damage';break;}
        if(s.health<=35 || s.fatigue>=80) {stop='rest or treatment needed';break;}
        if(previousHour<20 && s.hour>=20) {stop='nightfall';break;}
      }
      const complete=s.position===args.id;
      const message=`Journey ${complete?'complete':'paused'} after ${moved.length} step${moved.length===1?'':'s'}${complete?'.':`: ${stop}. Select the destination again to continue.`}`;
      log(s,message,complete?'good':'info');
      return {ok:true,message,complete,moved,stop,transitions};
    }
    const t = s.map[s.position]; let hours = 0, completingStage=0, completingSearch=false, injured=false, completingArmory=false;const transitions=[];
    switch (action) {
      case 'expeditionSize':s.expedition.size=args.count;return {ok:true,message:'Expedition size updated. The named roster below is the departure plan.'};
      case 'weaponLoadout':s.expedition.weapons=args.key;return {ok:true,message:P.weaponModes[args.key]+'. Applied at the next departure.'};
      case 'armorLoadout':s.expedition.armor=args.key;return {ok:true,message:P.armorModes[args.key]+'. Applied at the next departure.'};
      case 'buyArmor':s.resources.scrap-=P.armor[args.key].cost;s.armor[args.key]++;hours=2;log(s,`${P.armor[args.key].name} added to home stores. Best armor assigns one item per survivor at departure.`, 'good');break;
      case 'equip': s.armory.equipped=args.key;return {ok:true,message:`${A.weapons[args.key].name} equipped. ${A.policies[s.armory.policy].name} policy remains active.`};
      case 'policy': s.armory.policy=args.key;return {ok:true,message:`${A.policies[args.key].name}: ${A.policies[args.key].desc}`};
      case 'buyWeapon':
        s.resources.scrap-=A.weapons[args.key].cost;s.armory.owned[args.key]++;hours=2;
        log(s,`${A.weapons[args.key].name} acquired. Buy matching ammunition and equip it in Armory.`, 'good');break;
      case 'buyAmmo':
        s.resources.scrap-=A.ammo[args.key].cost;A.addAmmo(s,args.key,A.ammo[args.key].amount);hours=2;
        log(s,`Bought ${A.ammo[args.key].amount} ${A.ammo[args.key].name.toLowerCase()}.`, 'good');break;
      case 'upgradeArmory':
        s.resources.scrap-=A.upgradeCost(s).scrap;hours=4+2*s.armory.level;completingArmory=true;s.fatigue+=6;
        log(s,'Upgrading the supply station. Daily ammunition recovery begins after completion.');break;
      case 'move':
        if(!s.expedition.active) {
          transitions.push(P.depart(s));
          log(s,`Expedition departing: ${s.expedition.active.members.map(m=>s.crew.find(c=>c.id===m.id).name).join(', ')}. ${P.homeCrew(s).length} survivors remain at home.`);
        }
        s.position = args.id; s.map[args.id].visits++; reveal(s,s.position,s.config.profession === 'ranger' ? 2 : 1);
        hours = S.travelHours(s); s.fatigue += S.travelFatigue(s); s.noise += 3;
        log(s, `Arrived at ${s.map[args.id].name}.`, 'info'); injured=encounter(s,.7)>0; break;
      case 'scavenge': {
        const loot = {}; const multiplier = (1+P.searchBonus(s)) * (s.config.profession === 'scavenger' ? 1.35 : 1) * (s.fatigue > 75 ? .75 : 1) * (s.morale < 30 ? .8 : s.morale >= 85 ? 1.1 : 1);
        for (const k of resources) loot[k] = Math.round(t.stock[k] * multiplier);
        const equipment=A.salvage(s,t,random);const protection=P.salvage(s,t,random);if(protection)equipment.push(protection);
        if(equipment.length)log(s,`Armory salvage: ${equipment.join(', ')}. Equip recovered weapons in Armory.`, 'good');
        addResources(s,loot,1,true); t.stock = Object.fromEntries(resources.map(k=>[k,0])); t.searched = true; s.stats.searched++;
      ...
      case 'forage': {
        ...
        addResources(s,found,1,true); hours = 4; s.fatigue += 10; s.noise += 4;
      ...
      case 'expand':
        completingStage=s.baseStage+1;
        pay(s,S.stages[completingStage].cost); hours=S.stages[completingStage].hours;
        s.fatigue+=15; s.noise+=15;
        log(s,`Work begins on Stage ${completingStage}: ${S.stages[completingStage].name}. Existing production supplies the camp during construction.`); break;
      case 'recruit':
        pay(s,{food:4,water:3,fuel:1});
        s.recruitment={dueAt:S.absoluteHour(s)+S.arrivalDelay(s)};
        hours=2; s.noise+=12;
        log(s,`Recruitment call answered. One survivor will arrive in ${S.arrivalDelay(s)} game hours from the start of the call. A bed is reserved; time advances through actions, not real-world waiting.`, 'good'); break;
      case 'seekSurvivors':
        pay(s,{food:4,water:3}); hours=6; s.fatigue+=12; completingSearch=true;
        log(s,'A survivor search sets out. One recruit will join after six game hours if the expedition survives.'); encounter(s,.5); break;
      case 'broadcast':
      ...
      case 'trade': {
        const cost = tradeCost(s,args.key); pay(s,cost); const amount = (args.key === 'medicine' ? 3 : args.key === 'fuel' ? 4 : 7) * s.baseStage;
    ...
    s.actionCount++; advance(s,hours);
    if(alive(s)&&completingArmory){s.armory.level++;s.stats.built++;log(s,`Supply station level ${s.armory.level} completed. Daily recovered ammunition is shown in Armory.`, 'good');}
    if(alive(s) && completingStage) {
      s.baseStage=completingStage; s.stats.built++;
      log(s,`Stage ${s.baseStage}: ${S.stages[s.baseStage].name} completed. All earlier upgrades and stage benefits remain. New construction limits and daily production are active.`, 'good');
    }
    if(alive(s) && completingSearch) {
      const person=recruit(s); s.stats.recruited++; s.nextRecruitAt=S.absoluteHour(s)+24;
      log(s,`${person.name} joined after the survivor search. Assign a role in Survivors. Recruitment can be organized again in 24 game hours.`, 'good');
    }
    if(s.position===s.base&&s.expedition.active) {
      const manifest=P.returnHome(s);
      if(alive(s)){transitions.push(manifest);log(s,`Expedition home: ${manifest.members.length} survivors. Unspent recovered supplies: ${Object.entries(manifest.cargo).filter(([,n])=>n).map(([k,n])=>`${n} ${k}`).join(', ')||'none'}. Equipment and remaining ammunition unloaded.`, 'good');}
    }
    if (alive(s)) {checkWin(s); maybeEvent(s);}
    return {ok:true,injured,transitions,message:s.over ? 'The expedition has ended.' : 'Action complete.'};
  }
  function choiceStatus(s,index) {
    ...
    if (c.effect === 'recruit' && S.freeBeds(s)<=0) return 'Build more living quarters first.';
    return '';
  }
  function resolveEvent(s,index) {
    ...
    if (c.gain) addResources(s,c.gain,1,true); if (c.loss) addResources(s,c.loss,-1);
  ...
  function validate(s, legacy=false) {
    const legacyVersion=legacy===true?1:legacy===2?2:legacy===3?3:0;
    ...
    if (!object(s) || s.version !== (legacyVersion||VERSION) || !object(s.config)) fail();
    if (legacyVersion!==1) {
      if(!int(s.baseStage,1,5) || !int(s.nextRecruitAt,0,24000100)) fail();
      if(s.recruitment!==null && (!object(s.recruitment) || Object.keys(s.recruitment).length!==1 || !int(s.recruitment.dueAt,0,24000100))) fail();
    }
    if((!legacyVersion||legacyVersion===3)&&(!A.validate(s.armory,legacyVersion===3)||s.armory.level>Math.min(3,s.baseStage)))fail();
    ...
    if (!object(s.camp) || !Object.keys(structures).every(k=>int(s.camp[k],0,legacyVersion===1?structures[k].max:S.maxLevel(s,k))) || Object.keys(s.camp).some(k=>!Object.hasOwn(structures,k))) fail();
    if (!Array.isArray(s.crew) || s.crew.length<1 || s.crew.length>(legacyVersion===1?2+2*s.camp.beds:capacity(s))) fail();
    if(legacyVersion!==1 && s.recruitment && (s.crew.length>=capacity(s) || !s.camp.radio || (!s.over && (s.recruitment.dueAt<=S.absoluteHour(s) || s.recruitment.dueAt>S.absoluteHour(s)+12)))) fail();
    const ids = new Set();
    ...
    if(!legacyVersion&&!P.validate(s))fail();
    return true;
  }
  ...
  function deserialize(text) {
    ...
    if(s && s.version===1) {
      validate(s,true);
      s.version=2; s.baseStage=1; s.recruitment=null; s.nextRecruitAt=0;
      log(s,'Save upgraded to v2: existing buildings, survivors and supplies preserved. Base stages, diagonal journeys and direct recruitment are now available.', 'good');
    }
    if(s && s.version===2){validate(s,2);s.version=3;s.armory=A.initial();log(s,'Save upgraded to v3: world and pending recruitment preserved. A melee kit is available; buy or salvage firearms and ammunition in Armory.','good');}
    if(s&&s.version===3){validate(s,3);P.migrate(s);s.version=VERSION;log(s,'Save upgraded to v4: armor, named expedition parties and separate equipment reservations are available. Your world and existing gear are preserved.','good');}
    validate(s); return s;
  }
  return {VERSION,WIDTH,HEIGHT,expeditions:P,armory:A,systems:S,defs:{stages:S.stages,scenarios,eras,origins,professions,regions,tiles,structures,eventDefs,roles,resources,defaults},newGame,act,status,resolveEvent,choiceStatus,costToBuild,tradeCost,production,consumption,danger,capacity,atHome,neighbors,distance,tune,serialize,deserialize,validate};
});
