...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.AfterlightArmory=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function addAmmo(s,key,n){const before=s.armory.ammo[key];s.armory.ammo[key]=Math.min(9999,before+n);return s.armory.ammo[key]-before;}
  function status(s,action,args={}){
    ...
    if(action==='buyWeapon'){
      ...
      if(a.owned[args.key])return 'This weapon is already in your armory.';
    ...
    if(action==='buyAmmo'){
      ...
      if(a.ammo[args.key]+pack.amount>9999)return 'There is not enough storage for a full ammunition pack.';
  ...
  function salvage(s,t,random){
    ...
    if(pack[t.type]){const [key,n]=pack[t.type];const count=addAmmo(s,key,Math.max(1,Math.round((n+Math.floor(random(s)*4))*factor)));if(count)found.push(`${count} ${ammo[key].name.toLowerCase()}`);}
    if(['garage','shelter','industrial'].includes(t.type)&&random(s)<.16){
      ...
      if(!s.armory.owned[key]){s.armory.owned[key]=1;found.push(weapons[key].name);}
    }
    return found;
  }
  function validate(a){
    ...
    return keys(a,['owned','equipped','ammo','policy','level','spent','contacts'])&&keys(a.owned,Object.keys(weapons))&&Object.values(a.owned).every(n=>int(n,0,1))&&a.owned.melee===1&&Object.hasOwn(weapons,a.equipped)&&a.owned[a.equipped]===1&&keys(a.ammo,Object.keys(ammo))&&Object.values(a.ammo).every(n=>int(n,0,9999))&&Object.hasOwn(policies,a.policy)&&int(a.level,0,3)&&int(a.spent,0,100000000)&&int(a.contacts,0,100000000);
  }
  return {weapons,ammo,policies,initial,upgradeCost,daily,addAmmo,status,respond,salvage,validate};
});
