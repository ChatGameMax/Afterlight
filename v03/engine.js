...
(function (root, factory) {
  ...
  const VERSION = 3, WIDTH = 13, HEIGHT = 11;
  ...
  function encounter(s, factor = 1) {
    if (random(s) * 100 < danger(s) * .5 * factor) {
      const response=A.respond(s,integer(s, 4, 13) + s.fatigue / 18);
  ...
  function dawn(s) {
    ...
    if (s.config.threat > 0 && random(s) < s.config.threat / 700) {
      ...
      const response=A.respond(s,impact),hit=damage(s,response.damage);
      if(response.used)log(s,`Perimeter response used ${response.used} ${A.ammo[A.weapons[response.key].ammo].name.toLowerCase()}; +${response.noise} noise.`);
  ...
  function act(s, action, args = {}) {
    ...
    if (action==='travel') {
      const route=S.planRoute(s,args.id), moved=[];
      ...
      return {ok:true,message,complete,moved,stop};
    }
    const t = s.map[s.position]; let hours = 0, completingStage=0, completingSearch=false, injured=false, completingArmory=false;
    switch (action) {
      ...
      case 'buyWeapon':
        s.resources.scrap-=A.weapons[args.key].cost;s.armory.owned[args.key]=1;hours=2;
      ...
      case 'scavenge': {
        ...
        const equipment=A.salvage(s,t,random);
    ...
    return {ok:true,injured,message:s.over ? 'The expedition has ended.' : 'Action complete.'};
  }
  ...
  function validate(s, legacy=false) {
    const legacyVersion=legacy===true?1:legacy===2?2:0;
    ...
    if(!legacyVersion&&(!A.validate(s.armory)||s.armory.level>Math.min(3,s.baseStage)))fail();
  ...
  function deserialize(text) {
    ...
    if(s && s.version===2){validate(s,2);s.version=VERSION;s.armory=A.initial();log(s,'Save upgraded to v3: world and pending recruitment preserved. A melee kit is available; buy or salvage firearms and ammunition in Armory.','good');}
    validate(s); return s;
  }
  return {VERSION,WIDTH,HEIGHT,armory:A,systems:S,defs:{stages:S.stages,scenarios,eras,origins,professions,regions,tiles,structures,eventDefs,roles,resources,defaults},newGame,act,status,resolveEvent,choiceStatus,costToBuild,tradeCost,production,consumption,danger,capacity,atHome,neighbors,distance,tune,serialize,deserialize,validate};
});
